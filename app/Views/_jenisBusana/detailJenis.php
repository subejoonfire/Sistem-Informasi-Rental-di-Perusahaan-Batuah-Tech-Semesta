...
<section>
    ...
    <div class="p-1 grow">
        ...
        <form action="<?= base_url('/jenis/setJenisBusana/' . $row->id_jenis) ?> " method="POST" class="w-full">
            <?= csrf_field(); ?>
            <div class="flex w-full gap-5">
                <div class="form-control mb-2 w-1/2">
                    <label class="label">Nama Jenis</label>
                    <input name="nama_jenis" value="<?= $row->nama_jenis ?>" type="text" placeholder="Harga Sewa" class="input input-bordered" />
                </div>
            </div>
